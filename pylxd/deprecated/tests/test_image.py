...
class LXDAPIImageTestObject(LXDAPITestBase):
    ...
    def test_list_images(self, method, args, call_args, ms):
        ...
        self.assertEqual(["trusty"], getattr(self.lxd, "image_" + method)(*args))
    ...
    def test_list_images_fail(self, method, args, call_args, ms):
        ...
        self.assertRaises(
            exceptions.PyLXDException, getattr(self.lxd, "image_" + method), *args
        )
        ms.assert_called_once_with("GET", "/1.0/images", *call_args)
    ...
    @annotated_data(
        ...
        ("PyLXDException", exceptions.PyLXDException, exceptions.PyLXDException),
    )
    ...
    dates_data = (
        (
            "upload",
            (datetime.datetime.fromtimestamp(1435669853).strftime("%Y-%m-%d %H:%M:%S")),
        ),
        ("create", "Unknown"),
        ("expire", "Unknown"),
    )
...
class LXDAPIImageTestStatus(LXDAPITestBase):
    ...
    def test_image_operations(self, method, http, path, args, call_args, ms):
        self.assertTrue(getattr(self.lxd, "image_" + method)(*args))
        ms.assert_called_once_with(http, "/1.0/images" + path, *call_args)

    @annotated_data(*operations_data)
    def test_image_operations_fail(self, method, http, path, args, call_args, ms):
        ms.side_effect = exceptions.PyLXDException
        self.assertRaises(
            exceptions.PyLXDException, getattr(self.lxd, "image_" + method), *args
        )
...
class LXDAPIImageTestRaw(LXDAPITestBase):
    ...
    def test_image_export_fail(self, ms):
        ms.side_effect = exceptions.PyLXDException
        self.assertRaises(exceptions.PyLXDException, self.lxd.image_export, "fake")
        ms.assert_called_once_with("GET", "/1.0/images/fake/export")
...
class LXDAPIImageInfoTest(TestCase):
    ...
    info_list = (
        ...
        (
            "fingerprint",
            "04aac4257341478b49c25d22cea8a6ce" "0489dc6c42d835367945e7596368a37f",
        ),
        ("architecture", "x86_64"),
    )
    ...
    def test_info_no_data(self, method, expected, mc):
        self.assertEqual(
            expected,
            (getattr(self.image, "get_image_" + method)("test-image", data=None)),
        )
    ...
    def test_info_data(self, tag, method, metadata, expected, mc):
        self.assertEqual(
            expected,
            getattr(self.image, "get_image_" + method)("test-image", data=metadata),
        )
    ...
    @annotated_data(
        ...
        ("architecture_invalid", "architecture", {"architecture": -1}, KeyError),
    )
