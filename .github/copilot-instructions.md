...
Full details are in `doc/source/contributing.rst`. The short version:

1. Bump `version` in `pyproject.toml` and merge via PR.
2. Build the archives: `python3 -m build`
3. Verify: `twine check --strict dist/*`
4. Upload: `twine upload dist/*`
5. Create a GitHub release and tag using the autogenerated release notes from merged PRs.