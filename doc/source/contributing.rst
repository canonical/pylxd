...
2. Create a new release and tag on GitHub using the autogenerated release notes
   ...
   which builds and verifies the archives, generates a `build provenance
   attestation
   ...
   that cryptographically links them to the source commit, and publishes them
   to PyPI automatically via `Trusted Publisher (OIDC)
   <https://docs.pypi.org/trusted-publishers/>`_.  Once the workflow completes
   the new release is visible at https://pypi.org/project/pylxd/.
